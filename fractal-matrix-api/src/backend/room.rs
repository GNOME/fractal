...
pub fn get_room_detail(bk: &Backend, roomid: String, key: String) -> Result<(), Error> {
    ...
    get!(
        ...
        |r: JsonValue| {
            ...
            let _ = tx.send(BKResponse::RoomDetail(Ok((roomid, key, value))));
        },
        |err| {
            let _ = tx.send(BKResponse::RoomDetail(Err(err)));
        }
    );
...
pub fn get_room_avatar(bk: &Backend, roomid: String) -> Result<(), Error> {
    ...
    get!(
        ...
        |r: JsonValue| {
            ...
            let _ = tx.send(BKResponse::RoomAvatar(Ok((roomid, avatar))));
        },
        |err: Error| match err {
            Error::MatrixError(ref js)
                if js["errcode"].as_str().unwrap_or_default() == "M_NOT_FOUND" =>
            {
                let _ = tx.send(BKResponse::RoomAvatar(Ok((roomid, None))));
            }
            _ => {
                let _ = tx.send(BKResponse::RoomAvatar(Err(err)));
            }
        }
    );
    ...
    get!(
        ...
        |r: JsonValue| {
            ...
            let ms: Vec<Member> = joined
                ...
                .collect();
            let _ = tx.send(BKResponse::RoomMembers(Ok((roomid, ms))));
        },
        |err| {
            let _ = tx.send(BKResponse::RoomMembers(Err(err)));
        }
    );
...
pub fn get_room_messages(bk: &Backend, roomid: String, from: String) -> Result<(), Error> {
    ...
    get!(
        ...
        |r: JsonValue| {
            ...
            let _ = tx.send(BKResponse::RoomMessagesTo(Ok((list, roomid, prev_batch))));
        },
        |err| {
            let _ = tx.send(BKResponse::RoomMessagesTo(Err(err)));
        }
    );
    ...
    get!(
        ...
        |r: JsonValue| {
            ...
            if ms.is_empty() && id.is_some() {
                ...
                let _ = tx.send(BKResponse::RoomMessagesTo(Ok((ms, roomid, None))));
            }
        },
        |err| {
            let _ = tx.send(BKResponse::RoomMessagesTo(Err(err)));
...
pub fn send_msg(bk: &Backend, msg: Message) -> Result<(), Error> {
    ...
    query!(
        ...
        move |js: JsonValue| {
            ...
            let _ = tx.send(BKResponse::SentMsg(Ok((msg.id, evid.to_string()))));
        },
        |_| {
            let _ = tx.send(BKResponse::SentMsg(Err(Error::SendMsgError(msg.id))));
        }
    );
...
pub fn redact_msg(bk: &Backend, msg: &Message) -> Result<(), Error> {
    ...
    query!(
        ...
        move |js: JsonValue| {
            let evid = js["event_id"].as_str().unwrap_or_default();
            let _ = tx.send(BKResponse::SentMsgRedaction(Ok((msgid, evid.to_string()))));
        },
        |_| {
            let _ = tx.send(BKResponse::SentMsgRedaction(Err(
...
pub fn join_room(bk: &Backend, roomid: String) -> Result<(), Error> {
    ...
    post!(
        ...
        move |_: JsonValue| {
            ...
            let _ = tx.send(BKResponse::JoinRoom(Ok(())));
        },
        |err| {
            let _ = tx.send(BKResponse::JoinRoom(Err(err)));
        }
    );
...
pub fn leave_room(bk: &Backend, roomid: &str) -> Result<(), Error> {
    ...
    post!(
        &url,
        move |_: JsonValue| {
            let _ = tx.send(BKResponse::LeaveRoom(Ok(())));
        },
        |err| {
            let _ = tx.send(BKResponse::LeaveRoom(Err(err)));
        }
    );
...
pub fn mark_as_read(bk: &Backend, roomid: &str, eventid: &str) -> Result<(), Error> {
    ...
    post!(
        &url,
        move |_: JsonValue| {
            let _ = tx.send(BKResponse::MarkedAsRead(Ok((r, e))));
        },
        |err| {
            let _ = tx.send(BKResponse::MarkedAsRead(Err(err)));
        }
    );
...
pub fn set_room_name(bk: &Backend, roomid: &str, name: &str) -> Result<(), Error> {
    ...
    query!(
        ...
        &attrs,
        |_| {
            let _ = tx.send(BKResponse::SetRoomName(Ok(())));
        },
        |err| {
            let _ = tx.send(BKResponse::SetRoomName(Err(err)));
        }
    );
...
pub fn set_room_topic(bk: &Backend, roomid: &str, topic: &str) -> Result<(), Error> {
    ...
    query!(
        ...
        &attrs,
        |_| {
            let _ = tx.send(BKResponse::SetRoomTopic(Ok(())));
        },
        |err| {
            let _ = tx.send(BKResponse::SetRoomTopic(Err(err)));
        }
    );
...
pub fn set_room_avatar(bk: &Backend, roomid: &str, avatar: &str) -> Result<(), Error> {
    ...
    thread::spawn(move || {
        match put_media(mediaurl.as_str(), contents) {
            Err(err) => {
                let _ = tx.send(BKResponse::SetRoomAvatar(Err(err)));
            }
            Ok(js) => {
                ...
                put!(
                    ...
                    &attrs,
                    |_| {
                        let _ = tx.send(BKResponse::SetRoomAvatar(Ok(())));
                    },
                    |err| {
                        let _ = tx.send(BKResponse::SetRoomAvatar(Err(err)));
                    }
    ...
    thread::spawn(move || {
        if thumb != "" {
            match upload_file(&tk, &baseu, &thumb) {
                Err(err) => {
                    let _ = tx.send(BKResponse::AttachedFile(Err(err)));
                }
                Ok(thumb_uri) => {
        ...
        match upload_file(&tk, &baseu, &fname) {
            ...
            Ok(uri) => {
                ...
                let _ = tx.send(BKResponse::AttachedFile(Ok(msg)));
            }
        };
    });
...
pub fn new_room(
    ...
    post!(
        ...
        move |r: JsonValue| {
            ...
            r.name = Some(n);
            let _ = tx.send(BKResponse::NewRoom(Ok(r), internal_id));
        },
        |err| {
            let _ = tx.send(BKResponse::NewRoom(Err(err), internal_id));
        }
    );
    Ok(())
}
...
pub fn add_to_fav(bk: &Backend, roomid: String, tofav: bool) -> Result<(), Error> {
    ...
    query!(
        ...
        &attrs,
        |_| {
            let _ = tx.send(BKResponse::AddedToFav(Ok((roomid.clone(), tofav))));
        },
        |err| {
            let _ = tx.send(BKResponse::AddedToFav(Err(err)));
        }
    );
