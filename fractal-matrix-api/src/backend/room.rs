...
pub fn get_room_detail(bk: &Backend, roomid: String, key: String) -> Result<(), Error> {
    ...
    get!(
        ...
        |err| let _ = tx.send(BKResponse::RoomDetailError(err))
    );
...
pub fn get_room_members(bk: &Backend, roomid: String) -> Result<(), Error> {
    ...
    get!(
        ...
        |err| let _ = tx.send(BKResponse::RoomMembersError(err))
    );
...
fn parse_context(
    ...
    get!(
        ...
        |err| let _ = tx.send(BKResponse::RoomMessagesError(err))
    );
        ...
        match put_media(mediaurl.as_str(), contents) {
            ...
            Ok(js) => {
                ...
                put!(
                    &roomurl,
                    &attrs,
                    |_| let _ = tx.send(BKResponse::SetRoomAvatar),
                    |err| let _ = tx.send(BKResponse::SetRoomAvatarError(err))
                );
            }
        };
    });
