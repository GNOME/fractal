            ...
            file_chooser.connect_response(move |fcd, res| {
                if ResponseType::from(res) == ResponseType::Accept {
                    if fcd.get_filename().unwrap_or_default().exists() {
                        ...
                        let fcd_clone = fcd.clone();
                        let local_path_clone = local_path.clone();
                        confirm_dialog.connect_response(move |cd, res| {
                            if ResponseType::from(res) == gtk::ResponseType::Yes {
                                if let Err(_) = fs::copy(local_path_clone.clone(), fcd_clone.get_filename().unwrap_or_default()) {
