...
pub struct RoomHistory {
    ...
    sourceId: Rc<RefCell<Option<source::SourceId>>>,
}
...
impl RoomHistory {
    ...
    pub fn new(listbox: gtk::ListBox, op: &AppOp) -> RoomHistory {
        ...
        RoomHistory {
            ...
            room: op
                ...
                .clone(),
            divider: None,
            sourceId: Rc::new(RefCell::new(None)),
        }
    }
    ...
    pub fn create(&mut self, messages: Vec<MessageContent>) -> Option<()> {
        ...
        let sourceId = self.sourceId.clone();
        *self.sourceId.borrow_mut() = Some(gtk::idle_add(move || {
            ...
            if let Some(item) = data.pop() {
                ...
                if let Some(row) = create_row(&item, &room, has_header, backend.clone(), ui.clone())
                {
    ...
    pub fn destroy(self) {
        if let Some(id) = self.sourceId.borrow_mut().take() {
...
fn create_row(
    ...
    let widget = {
        ...
        let w = if has_header && row.mtype != RowType::Emote {
            Some(mb.widget())
        } else {
