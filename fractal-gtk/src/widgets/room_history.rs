...
impl RoomHistory {
    ...
    pub fn create(&mut self, messages: Vec<MessageContent>) -> Option<()> {
        ...
        gtk::idle_add(move || {
            ...
            if let Some(item) = data.pop() {
                ...
                if let Some(row) = create_row(&item, has_header, backend.clone(), ui.clone()) {
    ...
    pub fn add_new_message(&mut self, item: MessageContent) -> Option<()> {
        ...
        if let Some(row) = create_row(&item, has_header, self.backend.clone(), self.ui.clone()) {
            rows.push(item);
 ...
 * without header */
fn create_row(row: &MessageContent, has_header: bool, backend: Sender<BKCommand>, ui: UI) -> Option<gtk::ListBoxRow> {
