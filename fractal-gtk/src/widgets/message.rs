    ...
    pub fn create(&mut self, msg: &Message, has_header: bool, is_temp: bool) {
        ...
        let w = match msg.mtype {
            ...
            RowType::Video if is_temp => {
                upload_attachment_msg.add(&gtk::Label::new(Some("Uploading video.")));
                upload_attachment_msg
            }
            RowType::Audio if is_temp => {
                upload_attachment_msg.add(&gtk::Label::new(Some("Uploading audio.")));
                upload_attachment_msg
            }
            RowType::Image if is_temp => {
                upload_attachment_msg.add(&gtk::Label::new(Some("Uploading image.")));
                upload_attachment_msg
            }
            RowType::File if is_temp => {
                upload_attachment_msg.add(&gtk::Label::new(Some("Uploading file.")));
                upload_attachment_msg
            }
            _ if has_header => {
                self.row.set_margin_top(12);
