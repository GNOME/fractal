...
impl Data {
    ...
    fn set_context_menu_popover(&self, msg: &Message) {
        let widget = match &self.widget {
            Widget::Image(image) => Some(image.widget.clone().upcast()),
            Widget::Video(widget) => Some(widget.player.get_video_widget()),
            Widget::None => None,
        };
        if let Some(widget) = widget {
            ...
            let menu = MessageMenu::new(msg.id.as_str(), &mtype, &redactable, &widget, None);
